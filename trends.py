...
def analyze_tweet_sentiment(tweet):
    ...
    sumstr = 0
    countstr = 0
    strw = tweet_words(tweet)
    for elem in strw:
        s = get_word_sentiment(elem)
        if has_sentiment(s):
            countstr = countstr + 1
            sumstr = sumstr + sentiment_value(s)
    if countstr == 0:
        return make_sentiment(None)
    return make_sentiment(sumstr / countstr)
...
def find_centroid(polygon):
    ...
    area = 0
    i = 0
    lenght_p = len(polygon)
    while i < lenght_p - 1:
        area = area + (0.5 * ((latitude(polygon[i]) * longitude(polygon[i+1]) - (latitude(polygon[i+1]) * longitude(polygon[i])))))
        i = i + 1
    if area == 0 :
        ...
        i=0
    while i < lenght_p - 1:
        corx = corx + (1/6/area) * (latitude(polygon[i]) + latitude(polygon[i+1])) * ((latitude(polygon[i]) * longitude(polygon[i+1])) - (latitude(polygon[i+1]) * longitude(polygon[i])))
        cory = cory + (1/6/area) * (longitude(polygon[i]) + longitude(polygon[i+1])) * ((latitude(polygon[i]) * longitude(polygon[i+1])) - (latitude(polygon[i+1]) * longitude(polygon[i])))
        i = i + 1
    return (corx, cory, abs(area))
...
def average_sentiments(tweets_by_state):
    ...
    for elem in tweets_by_state:
        average = 0
        sums = 0
        total = 0
        for elm in tweets_by_state[elem]:
            if has_sentiment(analyze_tweet_sentiment(elm)):
                sums = sums + sentiment_value(analyze_tweet_sentiment(elm))
                total = total + 1
        if total != 0:
            average = sums / total
            averaged_state_sentiments[elem] = average
